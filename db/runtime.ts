...
const globalDatabase = globalThis as typeof globalThis & {
  clariorPostgres?: ReturnType<typeof postgres>;
};

function getClient() {
  const databaseUrl = process.env.DATABASE_URL;
  if (!databaseUrl) {
    throw new Error("DATABASE_URL não foi configurada para o PostgreSQL.");
  }

  globalDatabase.clariorPostgres ??= postgres(databaseUrl, {
    max: 5,
    idle_timeout: 20,
    connect_timeout: 10,
    prepare: false,
  ...
  return globalDatabase.clariorPostgres;
}
...
function withReturnedId(query: string) {
  ...
  return table && new Set(["assets", "findings", "products", "customers", "orders", "order_items", "order_events", "stock_movements", "audit_events"]).has(table)
    ? `${query} RETURNING id`
    : query;
}
...
export class PreparedStatement {
  ...
  async execute(executor: QueryExecutor = getClient() as unknown as QueryExecutor): Promise<QueryResult> {
    const query = withReturnedId(toPostgresQuery(this.query));
    const raw = await executor.unsafe(query, this.values) as DatabaseRow[] & { count?: number };
  ...
  async batch(statements: PreparedStatement[]) {
    return getClient().begin(async (transaction) => {
      ...
      for (const statement of statements) {
        results.push(await statement.execute(transaction as unknown as QueryExecutor));
      }
      return results;
    });
  }
